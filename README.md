...
信令与门面、libwebrtc 媒体、通话 UI 与 Demo 三屏。
`./scripts/test.sh` 六步全绿（53 个用例，纯 JVM，不需要设备），
并已在真机上跑通「免密登录 → 握手 → 建会议房 → 进房 → 媒体拉起 → 界面计时」。

**还没验的是真的出声出画**：当时的网络做了 AP 隔离（手机与 Mac 同网段互不可达），
信令只能走 `adb reverse` 的 USB 隧道，UDP 媒体过不去。换个能互通的网络接着验即可。
