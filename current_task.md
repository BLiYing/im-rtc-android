...
**任务一与任务二已落地（2026-09-05）：骨架 + 协议层 + 两个状态机，
...
3. **任务三（当前）**（**要等回调表冻结**）：`signaling/`（OkHttp WebSocket、握手、心跳、req_id 配对、退避重连、4401 三次上限）
   + 门面与回调表 + 日志回传。验收：真连本地服务端跑通进房离房（对齐 iOS 的 `LiveServerTests`）。
4. **任务四**：`call-engine-webrtc` 媒体 + 前台服务 + 音频焦点与路由。**真机验收**，
   且要与 Web、iOS 各互打一次。
5. **任务五**：`call-uikit`（来电横幅 / 1v1 四态 / 九宫格 / 悬浮球）+ **Demo App 三屏**
